...
**[Decent Network](https://decent.network)** 以“通讯”为第一性能力，在现有互联网之上构建身份驱动的可达网络，并在此之上承载存储与计算节点，最终支撑 App/Web 与 Agent 的规模化协作。
...
- **Friend Network（好友网络）**：基于身份建立的连接关系与可达路径  
...
示例：[Beagle Chat](https://beagle.chat)
...
[Decent Network](https://decent.network) 的第一性目标是建立“**身份可达的通讯网络**”：  
...
## 5. Layer 2：Decent Network SDK/Protocol（通讯优先）
...
## 6. Layer 1：载体（Phone / PC / Server / VM）

通讯可达性建立后，**载体**不再只是“客户端”，也可以成为可访问的资源提供者（存储/计算）。
资源的来源可以是：
- **个人设备**（手机/电脑）：可参与数据与算力协作  
- **私有服务器/虚拟机**：可承载长期运行的 Agent 与服务  
- **云节点**：仍然存在，但不再是唯一选择
